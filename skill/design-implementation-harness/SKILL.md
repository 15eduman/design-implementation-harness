...
2. Load the relevant prompt from `prompts/`.
...
6. Produce a QA report matching `schemas/qa-report.schema.json`.
...
- Create a QA draft:
  `python3 scripts/create_qa_report.py` if present, or
  `node scripts/create-qa-report.mjs`.
...
Use `agents.yaml` to choose the role:

- `source_reader`: source metadata and screenshot inventory.
